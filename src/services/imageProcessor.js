...
export class ImageProcessor {
  static async processImages(coverImages, backgroundImages, excelFile, onProgress) {
    // 验证图片数量
    if (coverImages.length !== backgroundImages.length) {
      throw new Error('背景素材图片的数量和封面图片的数量必须保持一致');
    }
    ...
    try {
      ...
      // 创建 ZIP 文件
      const zip = new JSZip();
      const totalSteps = coverImages.length;
      ...
      for (let i = 0; i < coverImages.length; i++) {
        ...
        // 处理封面图片
        const processedCover = await this.processImage(coverImages[i]);
        
        // 处理背景图片
        const backgroundCanvas = await this.processBackgroundImage(backgroundImages[i]);
        
        // 生成内页
        const contentPages = await this.generateContentPages(backgroundCanvas, textContent);

        // 获取文件夹编号并确保完成存储
        const nextIndex = await DBService.getNextAvailableIndex();
        await DBService.markIndexAsUsed(nextIndex);
        
        // 等待一小段时间确保 IndexedDB 操作完成
        ...
        folder.file('封面图.png', processedCover);
        
        // 添加内页
        contentPages.forEach((pageBlob, pageIndex) => {
          folder.file(`内页${pageIndex + 1}.png`, pageBlob);
        });
      }
  ...
  static async processImage(coverImage) {
    return new Promise((resolve) => {
      const img = new Image();
      img.onload = () => {
        const canvas = document.createElement('canvas');
  ...
  static async generateContentPages(backgroundCanvas, textContent) {
    // 随机打乱文字内容
    ...
    const createNewPage = () => {
      ...
      const titleMetrics = ctx.measureText('100份外企干货');
      ...
      ctx.fillText('100份外企干货', canvas.width / 2, 80 + titleHeight);
    ...
    while (currentTextIndex < shuffledContent.length) {
      ...
      if (willExceedLimit) {
        pages.push(await this.canvasToBlob(currentPage.canvas));
        currentPage = createNewPage();
      }
